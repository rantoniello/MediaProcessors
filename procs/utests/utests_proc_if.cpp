 ...
 * @brief Processor interface unit testing.
...
SUITE(UTESTS_PROC_IF)
{
    TEST(ALLOCATE_DUP_RELEASE_PROC_IF_T)
    {
        const proc_if_t proc_if_dummy_proc = {
                ...
                // dummy process frame
                [](proc_ctx_t*, fifo_ctx_t*, fifo_ctx_t*) -> int
                {
                    return STAT_SUCCESS;
                },
                // dummy option
                [](proc_ctx_t*, const char*, va_list) -> int
                {
                    return STAT_SUCCESS;
                },
                [](void*, void*, const void*, size_t, log_ctx_t*) -> void*
                {
                    return NULL;
                },
                [](void*, void**, size_t*, const void*, size_t, log_ctx_t*)
                -> int
                {
                    return STAT_SUCCESS;
                },
                [](const void*) -> proc_frame_ctx_t*
                {
                    return NULL;
                }
        };
         ...
         * 'proc_if_allocate()').
         */
        proc_if_t *proc_if = proc_if_dup(&proc_if_dummy_proc);
        CHECK(proc_if != NULL);
        ...
        for (int i = 0; i < 3/*Num. of data planes*/; i++) {
            for (int y = 0; y < (int)proc_frame_ctx->height[i]; y++) {
                for (int x = 0; x < (int)proc_frame_ctx->width[i]; x++) {
                    ...
                    CHECK(proc_frame_ctx->p_data[i][data_coord] ==
                            expected_val);
