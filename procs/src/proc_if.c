...
printf("############### %s:%s %d \n", __FILE__, __FUNCTION__, __LINE__); fflush(stdout); //FIXME!!
...
int proc_if_cmp(const proc_if_t* proc_if1, const proc_if_t* proc_if2)
{
    ...
    if(proc_if1->opt!= proc_if2->opt)
        goto end;
    if(proc_if1->iput_fifo_elem_ctx_memcpy_fxn!=
            proc_if2->iput_fifo_elem_ctx_memcpy_fxn)
        goto end;
    if(proc_if1->iput_fifo_elem_ctx_dequeue_fxn!=
            proc_if2->iput_fifo_elem_ctx_dequeue_fxn)
        goto end;
...
printf("####### %s:proc_if_fifo_elem_ctx_memcpy_default %d proc_frame_ctx push = %p\n", __FILE__, __LINE__, proc_frame_ctx); fflush(stdout); //FIXME!!
    /* Copy the pointer value (thus pass pointer address) */
    return memcpy(dest, &proc_frame_ctx, sizeof(void*));
}
        ...
        log_ctx_t *log_ctx)
{
    ...
    //*ref_elem = (void*)src; //FIXME!!
    memcpy(ref_elem, src, sizeof(void*));
    *ref_elem_size = size;
printf("######## %s:proc_if_fifo_elem_ctx_dequeue_default %d proc_frame_ctx dequeue = %p; size = %zu\n", __FILE__, __LINE__, *ref_elem, *ref_elem_size); fflush(stdout); //FIXME!!
    return STAT_SUCCESS;
}
